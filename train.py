...
model = YOLO("yolo11n.yaml").load("yolo11n.pt")  # load a pretrained utils (recommended for training)
# model.task = "classify"  # 强制指定任务为检测
print(f"Task: {model.task}")  # 输出推断的任务类型
results = model.train(data=r'C:\\learning\\MakeMoneyProject\\DetectCars\\data.yaml', project="./runs", epochs=100, imgsz=640, device=[], workers=0, batch=4, cache=True, amp=False)  # CPU 开始训练
...
utils	        None	    Specifies the utils file for training. Accepts a path to either a .pt pretrained utils or a .yaml configuration file. Essential for defining the utils structure or initializing weights.
...
epochs	        100	        Total number of training epochs. Each epoch represents a full pass over the entire dataset. Adjusting this value can affect training duration and utils performance.
...
imgsz	        640	        Target image size for training. All images are resized to this dimension before being fed into the utils. Affects utils accuracy and computational complexity.
save	        True	    Enables saving of training checkpoints and final utils weights. Useful for resuming training or utils deployment.
save_period	    -1	        Frequency of saving utils checkpoints, specified in epochs. A value of -1 disables this feature. Useful for saving interim models during long training sessions.
...
workers	        8	        Number of worker threads for data loading (per RANK if Multi-GPU training). Influences the speed of data preprocessing and feeding into the utils, especially useful in multi-GPU setups.
...
pretrained	    True	    Determines whether to start training from a pretrained utils. Can be a boolean value or a string path to a specific utils from which to load weights. Enhances training efficiency and utils performance.
optimizer	    'auto'	    Choice of optimizer for training. Options include SGD, Adam, AdamW, NAdam, RAdam, RMSProp etc., or auto for automatic selection based on utils configuration. Affects convergence speed and stability.
...
rect	        False	    Enables rectangular training, optimizing batch composition for minimal padding. Can improve efficiency and speed but may affect utils accuracy.
...
resume	        False	    Resumes training from the last saved checkpoint. Automatically loads utils weights, optimizer state, and epoch count, continuing training seamlessly.
...
profile 	    False	    Enables profiling of ONNX and TensorRT speeds during training, useful for optimizing utils deployment.
freeze	        None	    Freezes the first N layers of the utils or specified layers by index, reducing the number of trainable parameters. Useful for fine-tuning or transfer learning.
lr0	0.01	    Initial     learning rate (i.e. SGD=1E-2, Adam=1E-3) . Adjusting this value is crucial for the optimization process, influencing how rapidly utils weights are updated.
...
warmup_bias_lr	0.1	        Learning rate for bias parameters during the warmup phase, helping stabilize utils training in the initial epochs.
...
val	            True	    Enables validation during training, allowing for periodic evaluation of utils performance on a separate dataset.
plots	        False	    Generates and saves plots of training and validation metrics, as well as prediction examples, providing visual insights into utils performance and learning progression.
"""